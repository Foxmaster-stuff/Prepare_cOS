...
if=$(ip addr | sed -En 's/127.0.0.1//;s/.*inet (addr:)?(([0-9]*\.){3}[0-9]*).*/\2/p')
echo "Ip's on this computer is" $if
echo -n "Type your IP Address for VNC access: "
read ip
echo $ip
echo " "
echo -n "Type in VNC listening port, traffic must be allowed to this port if local firewall such as iptables is used: "
read port
...
echo "<domain type='kvm' id='34'>
  ...
  <devices>
    ...
    <disk type='file' device='disk'>
      ...
      <source file='$source_file/Core-clone.qcow2'/>
